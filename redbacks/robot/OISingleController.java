...
import redbacks.robot.actions.AcIntakeAnalog;

public class OISingleController extends OIBase {
	
	public static boolean climberReleased = false;
	
	private static CommandSetup
		intakeCubeAnalog = new CommandSetup(Robot.intake, new AcIntakeAnalog() {
			public double getSpeed() {
				double trigger = OI.axis_d_LT.get();
				return 0.3 + (trigger - 0.2) / 2;
			}
		}),
		outtakeCubeAnalog = new CommandSetup(Robot.intake, new AcIntakeAnalog() {
			public double getSpeed() {
				double trigger = OI.axis_d_RT.get();
	...
	public void mapOperations() {
		...
		//Operator Control
		whenHeld(d_LT, intakeCubeAnalog.c());
		whenHeld(d_RT, outtakeCubeAnalog.c());
		whenPressedReleased(d_Back, sideKickL.c(), solRetractIntakeL.c());
		whenPressedReleased(d_Start, sideKickR.c(), solRetractIntakeR.c());

		//Arm Positions
		whenPressed(d_B, armToBaseBack.c());
		whenPressed(d_POV_R, armToScaleBack.c());
		whenPressed(d_Y, armToTop.c());
		whenPressed(d_POV_L, armToScaleFront.c());
		whenPressed(d_X, armToBaseFront.c());

		whenPressedReleased(d_RB, highFirePrime.c(), highFireRelease.c());
	}
